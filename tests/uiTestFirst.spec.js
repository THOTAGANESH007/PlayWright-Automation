...
test("Child Window Handler", async ({ browser }) => {
  ...
  // textContent() is used whenever the dom contains the required result and we use inputValue() when the value gets in the runtime.

  const enteredInput = await userName.inputValue();
